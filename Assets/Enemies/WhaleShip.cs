...
public class WhaleShip : MonoBehaviour {
	...
	int dropTimer = dropTimerCountdownBase;
	Vector3 direction;
	...
	void FixedUpdate() {
		...
		Vector2 whalePoint = whaleShip.rigidbody.position;
		...
		if( (whalePoint - centerPoint).magnitude > 300 ) {
			...
			whaleShip.rigidbody.position = point;
		...
		whaleShip.rigidbody.transform.LookAt (center.transform, Vector3.back);
		whaleShip.rigidbody.velocity = new Vector3(0, 0, 0);
	...
	public void gotHit(){
		if( hitPoints > 0 ) {
			...
			Transform trans = whaleShip.GetComponent<Transform>();
			Vector3 scale = trans.localScale;
			Vector3 newScale = new Vector3(scale.x - 0.5f, scale.y - 0.5f, scale.z - 0.5f);
			trans.localScale = newScale;
		}
		else
		{
			GameObject.Destroy(whaleShip);
		}
